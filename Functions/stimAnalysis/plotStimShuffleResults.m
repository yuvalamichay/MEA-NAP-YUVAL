...
%
% Only generates plots for individual patterns (patternIdx must be provided).
% Y-axes use "Channel Index" labelling.
%
% INPUTS
% ------
...
% patternIdx     : scalar   – pattern index; plots are only generated when
%                             this is provided (non-empty).
%
...
% Only generate plots for individual patterns, not for all-patterns summary
...
numChannels = length(shuffleResults.AUC_obs);
AUC_obs = shuffleResults.AUC_obs;
pctile_lo = shuffleResults.pctile_lo;
pctile_hi = shuffleResults.pctile_hi;
isSignificant = shuffleResults.isSignificant;
xVec = 1:numChannels;

%% Figure 1: Null distribution heatmap with observed AUC overlay
fig1 = figure('Position', [100, 100, 800, 500], 'Color', 'w');

% Sort null distributions for each electrode for visualisation
AUC_null_sorted = sort(shuffleResults.AUC_null, 2);

subplot(2, 1, 1)
imagesc(1:shuffleResults.Nshuffles, 1:numChannels, AUC_null_sorted)
colorbar
ylabel('Channel Index')
xlabel('Shuffle (sorted)')
title(sprintf('Null AUC distribution – %s', titleSuffix))
set(gca, 'TickDir', 'out')
box off
...
nullMedian = median(shuffleResults.AUC_null, 2);
...
% Overlay observed
scatter(xVec(~isSignificant), AUC_obs(~isSignificant), 25, ...
    [0.3, 0.3, 0.7], 'filled', 'DisplayName', 'Not significant');
scatter(xVec(isSignificant), AUC_obs(isSignificant), 40, ...
